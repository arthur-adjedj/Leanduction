...
def withSparseParamIndType [Inhabited α] (info : InductiveVal) (positivityMask : Array Bool) (sparseIndNames : Array Name) /-(predUnivs : Array (Option Name))-/ (k : M α): MetaM α := do
  forallBoundedTelescope info.type info.numParams fun paramsFVars _ =>
  ...
  forallBoundedTelescope ctor.type info.numParams fun params ty => do
    ...
    forallTelescope ty fun args indAsDs => do
      ...
      withConstPreds ctx.paramsFVars args 0 #[] fun constPreds => do
        ...
        -- ty := PArgs -> IP As PAs Ds (c As args)
        let ty ← mkForallFVars (constPreds.filterMap id) ty
        -- ty := Args -> PArgs -> IP As PAs Ds (c As args)
        let ty ← mkForallFVars args ty
        -- ty := PAs -> Args -> PArgs -> IP As PAs Ds (c As args)
        let ty ← mkForallFVars (ctx.predsFVars.filterMap id) ty
        -- ty := As -> PAs -> Args -> PArgs -> IP As PAs Ds (c As args)
...
where
  ...
  /-- Given a (recursive, potentially nested) argument `arg`, return the corresponding predicate. -/
  predOfArg (arg: Expr) : MetaM (Option Expr) := do
    withTraceNode `Leanduction.Parametricity (fun e => return m!"predOfArg {arg} = {e.toOption}") do
    let argTy ← inferType arg
    let params := ctx.paramsFVars
    forallTelescopeReducing argTy fun conArgArgs conArgRes => do
      -- this could be made faster, TODO?
      unless conArgRes.hasAnyFVar (fun f => params.any (·.fvarId! == f)) || conArgRes.getUsedConstants.any info.all.contains do
        return none
      conArgRes.withApp fun fn' fnargs => do
      trace[Leanduction.Parametricity] m!"fnargs : {fnargs}"
      if let some p := fn'.fvarId? then
        let some predFVar := getParamPred? params p
          | return none
        ...
        let some fn := fn'.constName?
          | return none
        unless (← isInductive fn) do
          return none
        if let some idx := info.all.findIdx? (· == fn) then
          let ty := mkAppN ctx.sparseIndFVars[idx]! fnargs[:params.size]
          let ty := mkAppN ty (ctx.predsFVars.filterMap id)
          ...
          let nestedIndParams := fnargs[:nestedInd.numParams]
          trace[Leanduction.Parametricity] m!"nestedIndParams : {nestedIndParams}"
          let PAs ← nestedIndParams.toArray.zipIdx.mapM fun (nestedArgarg, idx) => do
            unless nestedParamsMask[idx]! do
              return none
            forallTelescope nestedArgarg fun xs ty => do
            forallTelescopeReducing (← inferType ty) fun missingIndices _ => do
              withLocalDeclD `z (← mkForallFVars xs (mkAppN ty missingIndices)) fun arg => do
                let pred ← do
                  if let some P ← predOfArg arg then
                    pure P
                  else
                    pure (mkConst ``True [])
                let P ← mkLambdaFVars (missingIndices.push arg) pred
                return some P
          ...
          -- Is As PAs Ds
          let ty := mkAppN ty fnargs[nestedInd.numParams:]
          -- llargs -> Is As PAs Ds
          let ty := mkApp ty (mkAppN arg conArgArgs)
          -- llargs -> Is As PAs Ds
          let ty ← mkForallFVars conArgArgs ty
